...
|`\b`|Em uma `[`  `]` classe de caractere character_group, corresponde a um backspace, `\u0008` .  (Consulte [classes de caractere](character-classes-in-regular-expressions.md).) Fora de uma classe de caractere, `\b` é uma âncora que corresponde a um limite de palavra. (Confira [Âncoras](anchors-in-regular-expressions.md)).|  
